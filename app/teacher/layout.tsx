...
export default async function TeacherLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen flex">
      <nav className="w-56 bg-indigo-800 text-white flex flex-col gap-1 p-4">
        <p className="text-xs uppercase tracking-widest text-indigo-300 mb-4">老師後台</p>
        <Link href="/teacher/schedule" className="px-3 py-2 rounded hover:bg-indigo-700 text-sm">
          課表管理
        </Link>
        <Link href="/teacher/enrollments" className="px-3 py-2 rounded hover:bg-indigo-700 text-sm">
          預約名單
        </Link>
        <Link href="/teacher/scan" className="px-3 py-2 rounded hover:bg-indigo-700 text-sm">
          掃描簽到
        </Link>
        <Link href="/teacher/profile" className="px-3 py-2 rounded hover:bg-indigo-700 text-sm">
          個人檔案
        </Link>
      </nav>
